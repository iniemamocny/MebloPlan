...
export default function RoomTab({ three }:{ three: React.MutableRefObject<any> }){
  ...
  useEffect(()=>{ store.setRoom({ height }) }, [height])
  useEffect(()=>{
    const group = three.current?.group; if(!group) return
    const toRemove = group.children.filter((c:any)=>c.userData?.kind==='room')
    toRemove.forEach((m:any)=>{ group.remove(m); (m as any).geometry?.dispose?.(); Array.isArray((m as any).material)? (m as any).material.forEach((mm:any)=>mm.dispose()):(m as any).material?.dispose?.() })
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(12,12), new THREE.MeshStandardMaterial({ color:0xf0f0f0, side:THREE.DoubleSide }))
    floor.rotation.x = -Math.PI/2; (floor as any).userData.kind='room'; group.add(floor)
    ...
    store.room.walls.forEach((w,i)=>{
      ...
      const next = new THREE.Vector2(cursor.x + dir.x*len, cursor.y + dir.y*len)
      const mid = new THREE.Vector2((cursor.x+next.x)/2, (cursor.y+next.y)/2)
      const box = new THREE.Mesh(new THREE.BoxGeometry(len, h, 0.1), new THREE.MeshStandardMaterial({ color:0xd1d5db }))
      box.position.set(mid.x, h/2, mid.y); box.rotation.y = -ang; (box as any).userData.kind='room'; group.add(box)
      cursor = next
    })
  }, [store.room, three])
  const addWall = () => store.addWall({ length: Number(len)||0, angle: Number(angle)||0 })
  const addWindow = () => store.addOpening({ type:'window', wall:0, offset:500, width:900, height:1200, sill:900 })
