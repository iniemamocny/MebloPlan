...
export default function CutlistTab() {
  ...
  const detailedPack = useMemo(
    () => store.modules.map((m) => cutlistForModule(m, store.globals)),
    [store.modules, store.globals],
  );
  const detailed = useMemo(
    () => detailedPack.flatMap((p) => p.items),
    [detailedPack],
  );
  const edgeAll = useMemo(
    () => detailedPack.flatMap((p) => p.edges),
    [detailedPack],
  );
  ...
  const exportCSV = () => {
    download('cutlist_detailed.csv', toCSV(detailed), 'text/csv');
  };
  const exportCSVagg = () => {
    download('cutlist_aggregated.csv', toCSV(aggregated), 'text/csv');
  };
  const exportPDF = () => {
    ...
    doc.setFontSize(14);
    line('Lista formatek — szczegółowa (z frontami i szufladami)');
    doc.setFontSize(10);
    line('Moduł | Materiał | Element | Ilość | W (mm) | H (mm)');
    detailed.forEach((it) =>
      line(
        `${it.moduleLabel} (${it.moduleId}) | ${it.material} | ${it.part} | ${it.qty} | ${it.w} | ${it.h}`,
      ),
    );
    doc.addPage();
    y = 12;
    doc.setFontSize(14);
    line('Lista formatek — agregowana');
    doc.setFontSize(10);
    line('Materiał | Element | Ilość | W (mm) | H (mm)');
    aggregated.forEach((it) =>
      line(`${it.material} | ${it.part} | ${it.qty} | ${it.w} | ${it.h}`),
    );
    doc.addPage();
    y = 12;
    doc.setFontSize(14);
    line('Okleina — podsumowanie');
    doc.setFontSize(10);
    line('Materiał | Długość (mm) | Długość (mb)');
    edgeAgg.forEach((e) =>
      line(
        `${e.material} | ${Math.round(e.length)} | ${(e.length / 1000).toFixed(2)}`,
      ),
    );
  ...
  return (
    <div className="section">
      ...
      >
        ...
        >
          <div className="h2">
            Walidacja formatu {board.W}×{board.L}
          </div>
          <div className="small">
            {plan.ok
              ? `Arkusze: ${plan.sheets}`
              : 'Uwaga: formatka nie mieści się'}
          </div>
        </div>
        <div className="small">
          {board.hasGrain
      ...
      <MultiMaterialPreview
        ...
        W={board.W}
        kerf={board.kerf}
        hasGrain={board.hasGrain}
        items={items}
      />

      <div className="hd">
        <div>
          <div className="h1">Formatki (rozkrój)</div>
        </div>
      </div>
      <div className="bd">
        <div className="row" style={{ marginBottom: 8 }}>
          <button className="btn" onClick={exportCSV}>
            Eksport CSV — szczegółowy
          </button>
          <button className="btnGhost" onClick={exportCSVagg}>
            Eksport CSV — agregowany
          </button>
        ...
        <table className="table">
          ...
          <tbody>
            {edgeAgg.map((e, i) => (
              <tr key={i}>
                <td>{e.material}</td>
                <td>{Math.round(e.length)}</td>
                <td>{(e.length / 1000).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
