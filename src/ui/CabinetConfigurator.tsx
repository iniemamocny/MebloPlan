...
const CabinetConfigurator: React.FC<Props> = ({
  ...
  return (
    <div className="section">
      <div className="hd">
        <div>
          <div className="h1">Konfiguracja — {variant.label}</div>
        </div>
        <div className="tabs">
          <button
            className={`tabBtn ${cfgTab === 'basic' ? 'active' : ''}`}
            onClick={() => setCfgTab('basic')}
          >
            Podstawowe
          </button>
          <button
            className={`tabBtn ${cfgTab === 'adv' ? 'active' : ''}`}
            onClick={() => setCfgTab('adv')}
          >
            Zaawansowane
          </button>
        </div>
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      const v =
                        Number((e.target as HTMLInputElement).value) || 0;
                      if (v > 0) onAdd(v, gLocal);
                    }
                  }}
                />
              </div>
              <div className="row" style={{ alignItems: 'flex-end' }}>
                <button className="btn" onClick={() => onAdd(widthMM, gLocal)}>
                  Wstaw szafkę
                </button>
              </div>
            </div>
              ...
              <TechDrawing
                ...
                drawers={
                  variant?.key?.startsWith('s')
                    ? Number(variant.key.slice(1))
                    : variant?.key?.includes('+drawer')
                      ? 1
                      : 0
                }
        ...
        {cfgTab === 'adv' && (
          <div>
            <div className="grid4">
              ...
              <div>
                <div className="small">Głębokość (mm)</div>
                <input
                  className="input"
                  type="number"
                  value={gLocal.depth}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAdv({
                      ...gLocal,
                      backPanel: (e.target as HTMLSelectElement).value,
                    })
                  }
                >
            ...
            <div style={{ marginTop: 8 }}>
              <div className="small">
                Szczeliny i wysokości frontów (ustawiaj graficznie)
              </div>
              <TechDrawing
                mode="edit"
                family={family}
            ...
            <div className="row" style={{ marginTop: 8 }}>
              ...
              <button className="btnGhost" onClick={() => setCfgTab('basic')}>
                ← Podstawowe
              </button>
            </div>
          </div>
