...
export default function App(){
  ...
  useEffect(()=>{
    ...
    setAdv({ height:g.height, depth:g.depth, boardType:g.boardType, frontType:g.frontType, boardThickness:g.boardThickness, gaps:{...g.gaps}, shelves:g.shelves ?? defaultShelves })
   ...
   * local origin.  Back thickness is fixed at 3 mm while board
   * thickness comes from module or global settings.  Front splits are determined by the
  ...
  const createCabinetMesh = (mod: any) => {
    ...
    const T = (mod.adv?.boardThickness ?? store.globals[mod.family]?.boardThickness ?? 18) / 1000
    const backT = 0.003
  ...
  const onAdd = (widthMM:number, advLocal:any) => {
    ...
    const price = computeModuleCost({ family, kind:kind.key, variant:variant.key, width: widthMM, adv:{ height:g.height, depth:g.depth, boardType:g.boardType, frontType:g.frontType, boardThickness:g.boardThickness, gaps:g.gaps } })
  ...
  const doAutoOnSelectedWall = () => {
    ...
    placed.forEach((pl,i)=>{
      ...
      const price = computeModuleCost({ family, kind:(KIND_SETS[family][0]?.key)||'doors', variant:'d1', width: wmm, adv:{ height:g.height, depth:g.depth, boardType:g.boardType, frontType:g.frontType, boardThickness:g.boardThickness, gaps:g.gaps } })
                  ...
                  <div>
                    ...
                    <div style={{marginTop:8}}>
                      <TechDrawing
                        ...
                        boardThickness={gLocal.boardThickness}
                        gaps={gLocal.gaps}
                ...
                {cfgTab==='adv' && (
                  <div>
                    <div className="grid4">
                      ...
                      <div><div className="small">Grubość płyty (mm)</div><input className="input" type="number" value={gLocal.boardThickness} onChange={e=>setAdv({...gLocal, boardThickness:Number((e.target as HTMLInputElement).value)||0})} /></div>
                    </div>
