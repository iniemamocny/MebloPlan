...
export default function App() {
  ...
  const [boardKerf, setBoardKerf] = useLocalStorageState<number>(
    'boardKerf',
    3,
  );
  const [boardHasGrain, setBoardHasGrain] = useLocalStorageState<boolean>(
    'boardHasGrain',
    true,
  );
      ...
      <div className="canvasWrap">
        ...
        <div className="topbar row">
          <button
            className="btnGhost"
            onClick={() =>
              store.setRole(store.role === 'stolarz' ? 'klient' : 'stolarz')
            }
          >
          ...
          <select
            ...
            value={selWall}
            onChange={(e) =>
              setSelWall(Number((e.target as HTMLSelectElement).value) || 0)
            }
          >
            {getWallSegments().map((s, i) => (
              <option key={i} value={i}>
                Ściana {i + 1} ({Math.round(s.length)} mm)
              </option>
            ))}
          </select>
      ...
      <aside className="sidebar">
        <div className="section card">
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'end' }}>
              ...
              >
                <input
                  type="checkbox"
                  checked={boardHasGrain}
                  onChange={(e) => setBoardHasGrain(e.target.checked)}
                />
                Płyta ma usłojenie
              </label>
            </div>
          ...
          <div className="tiny muted" style={{ marginTop: 6 }}>
            Bez usłojenia: formatki można obracać (muszą się zmieścić w {boardL}
            ×{boardW} lub {boardW}×{boardL}). Z usłojeniem: elementy wymagające
            słojów bez rotacji (h≤{boardL}, w≤{boardW}).
          </div>
        </div>
        ...
        <div className="tabs">
          <button
            className={`tabBtn ${tab === 'cab' ? 'active' : ''}`}
            onClick={() => setTab('cab')}
          >
            Kuchnia
          </button>
          <button
            className={`tabBtn ${tab === 'room' ? 'active' : ''}`}
            onClick={() => setTab('room')}
          >
            Pomieszczenie
          </button>
          <button
            className={`tabBtn ${tab === 'costs' ? 'active' : ''}`}
            onClick={() => setTab('costs')}
          >
            Koszty
          </button>
          <button
            className={`tabBtn ${tab === 'cut' ? 'active' : ''}`}
            onClick={() => setTab('cut')}
          >
            Formatki
          </button>
          ...
          <>
            ...
            <div className="section">
              <div className="hd">
                <div>
                  <div className="h1">
                    Podkategorie ({FAMILY_LABELS[family]})
                  </div>
                </div>
              </div>
              <div className="bd">
