...
type Props = {
  L: number, W: number, kerf: number, hasGrain: boolean,
  items: Agg[]
}
                ...
                return (
                  <div key={s.index} className="card" style={{padding:6}}>
                    ...
                    <svg viewBox={vb} width="100%" style={{maxHeight:380, border:'1px solid #e5e7eb', background:'#fff'}}>
                      ...
                      <defs>
                        <pattern id="grain-vert" width="8" height="8" patternUnits="userSpaceOnUse">
                          <rect width="4" height="8" fill="#e0e7ff" />
                          <rect x="4" width="4" height="8" fill="#c7d2fe" />
                        </pattern>
                        <pattern id="grain-horiz" width="8" height="8" patternUnits="userSpaceOnUse">
                          <rect width="8" height="4" fill="#e0e7ff" />
                          <rect y="4" width="8" height="4" fill="#c7d2fe" />
                        </pattern>
                        <pattern id="plain" width="1" height="1" patternUnits="userSpaceOnUse">
                      ...
                      {s.placed.map((p:any,i:number)=> {
                        ...
                        return (
                          <g key={i}>
                            <rect x={p.x} y={p.y} width={p._w} height={p._h} fill={`url(#${fillId})`} stroke="#1e40af" />
                            <circle cx={p.x + 14} cy={p.y + 14} r={11} fill="#111827" />
                            <text x={p.x + 14} y={p.y + 14} fontSize="12" textAnchor="middle" dominantBaseline="central" fill="#fff">{p.idx ?? (i+1)}</text>
                            <text x={p.x+4} y={p.y+p._h-6} fontSize="11" fill="#334155">{p._w}×{p._h}</text>
                          </g>
                        )
                      })}
                    </svg>
