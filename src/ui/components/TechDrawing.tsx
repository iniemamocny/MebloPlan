...
function DimH({ x1,y, x2, label }:{ x1:number; y:number; x2:number; label:string }){
  const arrow=4
  return (
    <g>
      <line x1={x1} y1={y} x2={x2} y2={y} stroke="#111" strokeWidth={1}/>
      <polyline points={`${x1},${y} ${x1+arrow},${y-arrow} ${x1+arrow},${y+arrow}`} fill="#111"/>
      <polyline points={`${x2},${y} ${x2-arrow},${y-arrow} ${x2-arrow},${y+arrow}`} fill="#111"/>
      <text x={(x1+x2)/2} y={y-6} fontSize="11" textAnchor="middle" dominantBaseline="middle" fill="#111">{label}</text>
    </g>
  )
}
function DimV({ x, y1, y2, label, left=false }:{ x:number; y1:number; y2:number; label:string; left?:boolean }){
  const arrow=4
  return (
    <g>
      <line x1={x} y1={y1} x2={x} y2={y2} stroke="#111" strokeWidth={1}/>
      <polyline points={`${x},${y1} ${x-arrow},${y1+arrow} ${x+arrow},${y1+arrow}`} fill="#111"/>
      <polyline points={`${x},${y2} ${x-arrow},${y2-arrow} ${x+arrow},${y2-arrow}`} fill="#111"/>
      <text x={left? x-8 : x+8} y={(y1+y2)/2} fontSize="11" textAnchor={left?'end':'start'} dominantBaseline="middle" fill="#111">{label}</text>
    </g>
  )
}

export default function TechDrawing({
  mode, widthMM, heightMM, depthMM, boardType, boardThicknessMM, gaps, drawers, drawerFronts, onChangeGaps, onChangeDrawerFronts
}:Props){
  const W = 360, H = 230
  const pad = 14
  const t = boardThicknessMM ?? parseThickness(boardType)
  ...
  const drawerSplits = useMemo(()=>{
    ...
    const arr = (drawerFronts && drawerFronts.length===drawers) ? drawerFronts : Array.from({length:drawers}, ()=> Math.floor(availFrontH/drawers))
    const sum = arr.reduce((a,b)=>a+b,0)
    const pxPerMM = front.h / Math.max(1, sum)
    let cum=0; const ys:number[] = []
    for (let i=0;i<drawers-1;i++){ cum += arr[i]; ys.push(front.y + cum*pxPerMM) }
    return ys
  ...
  const onMouseDown = (e: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    ...
    if (!tag) return
    setDrag({ tag, startY:e.clientY, startVals: { gaps:{...gaps}, splits: drawerSplits.slice() } })
  }
  const onMouseMove = (e: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    ...
    if (drag.tag==='gap-top'){
      ...
      const availFrontH = heightMM - (gaps.top+gaps.bottom)
      const arr = (drawerFronts && drawerFronts.length===drawers) ? drawerFronts.slice() : Array.from({length:drawers}, ()=> Math.floor(availFrontH/drawers))
      const sum = arr.reduce((a,b)=>a+b,0)
      const pxPerMM = front.h / Math.max(1, sum)
      ...
      const rest = arr.reduce((s,n,i)=> i===idx||i===idx+1 ? s : s+n, 0)
      const total = rest + a + b
      const scale = (sum - rest) / (a + b)
      a = Math.round(a*scale); b = Math.round(b*scale)
      const res = arr.slice(); res[idx]=a; res[idx+1]=b
  ...
  return (
    <div className="row" style={{gap:12, alignItems:'flex-start'}}>
      ...
      <svg width={W} height={H} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} style={{border:'1px solid #E5E7EB', borderRadius:8, background:'#fff'}}>
        ...
        {/* korpus */}
        <rect x={pad} y={pad} width={innerW} height={innerH} fill="url(#hatch)" stroke="#999"/>
        {/* front */}
        <rect x={front.x} y={front.y} width={front.w} height={front.h} fill="#e5e7eb" stroke="#666"/>
        {/* podziały szuflad */}
        {drawerSplits.map((y,i)=> (
          <g key={i}>
            <line x1={front.x} y1={y} x2={front.x+front.w} y2={y} stroke="#fff" strokeWidth={2}/>
            {mode==='edit' && <rect data-tag={`split-${i}`} x={front.x} y={y-4} width={front.w} height={8} fill="transparent" cursor="ns-resize" />}
          </g>
        ))}
        ...
        <DimH x1={front.x} y={pad-24} x2={front.x+front.w} label={`${innerClearW}`} />
      ...
      <svg width={W} height={H} style={{border:'1px solid #E5E7EB', borderRadius:8, background:'#fff'}}>
        ...
        {/* boki */}
        <rect x={pad} y={pad} width={8} height={innerH} fill="url(#hatch2)" stroke="#999"/>
        <rect x={W-pad-8} y={pad} width={8} height={innerH} fill="url(#hatch2)" stroke="#999"/>
        {/* plecy */}
        <rect x={pad+8} y={pad} width={2} height={innerH} fill="#999" opacity="0.5"/>
        {/* wieńce */}
        <rect x={pad+10} y={pad} width={innerW-20} height={8} fill="url(#hatch2)" stroke="#999"/>
        <rect x={pad+10} y={H-pad-8} width={innerW-20} height={8} fill="url(#hatch2)" stroke="#999"/>
        {/* pomocnicze półki (symbolicznie) */}
        {drawers>0 && (<>
          <rect x={pad+12} y={pad+innerH*0.35} width={innerW-24} height={6} fill="#d1d5db"/>
          <rect x={pad+12} y={pad+innerH*0.65} width={innerW-24} height={6} fill="#d1d5db"/>
        </>)}
        {/* wymiary głębokości */}
        <DimH x1={pad+10} y={pad-10} x2={W-pad-10} label={`${Math.round(depthMM)}`} />
        <DimH x1={pad+12} y={pad-24} x2={W-pad-12-2} label={`${Math.round(depthMM - (2 + t))}`} />
      </svg>
    </div>
  )
}
