          ...
          return (
            <div key={s.index} className="card" style={{padding:6}}>
              ...
              <svg viewBox={vb} width="100%" style={{maxHeight:360, border:'1px solid #e5e7eb', background:'#fff'}}>
                ...
                {s.placed.map((p,i)=> (
                  <g key={i}>
                    <rect x={p.x} y={p.y} width={p._w} height={p._h} fill="#dbeafe" stroke="#1e40af" />
                    <text x={p.x+4} y={p.y+14} fontSize="12" fill="#1e3a8a">{p.name}</text>
                    <text x={p.x+4} y={p.y+28} fontSize="11" fill="#334155">{p._w}×{p._h}</text>
                  </g>
                ))}
              </svg>
            </div>
