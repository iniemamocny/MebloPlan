...
export default function CabinetGraphic({
  ...
  const front = useMemo(() => {
    const gxL = (gaps.left || 0) / scale;
    const gxR = (gaps.right || 0) / scale;
    const gyT = (gaps.top || 0) / scale;
    const gyB = (gaps.bottom || 0) / scale;
    return {
      x: inner.x + gxL,
      y: inner.y + gyT,
      w: inner.w - gxL - gxR,
  ...
  const drawersHeights = useMemo(() => {
    ...
    const arr =
      drawerFronts && drawerFronts.length === drawers
        ? drawerFronts.slice()
        : Array.from({ length: drawers }, () => Math.floor(total / drawers));
    const s = arr.reduce((a, b) => a + b, 0);
    if (s !== total) {
      arr[drawers - 1] += total - s;
    }
    return arr;
  ...
  const onMouseDown = (e: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    ...
    setDrag({
      tag,
      startY: e.clientY,
      startVals: { gaps: { ...gaps }, drawers: drawersHeights.slice() },
    });
  };
  ...
  const onMouseMove = (e: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    ...
    if (drag.tag === 'gap-top') {
      ...
      const pxPerMM =
        ...
        Math.max(
          1,
          all.reduce((a: number, b: number) => a + b, 0),
        );
      ...
      const restTotal = drag.startVals.drawers.reduce(
        (s: number, n: number, i: number) =>
          i === idx || i === idx + 1 ? s : s + n,
        0,
      );
      const startTotal = drag.startVals.drawers.reduce(
      ...
      const res = drag.startVals.drawers.slice();
      res[idx] = a;
      res[idx + 1] = b;
  ...
  useEffect(() => {
    ...
    light.position.set(1.2, 1.5, 1.0);
    scene.add(light);
    ...
    if (drawers > 0) {
      const total = Math.max(
        50,
        Math.round(heightMM - (gaps.top + gaps.bottom)),
      );
      const arr =
        drawerFronts && drawerFronts.length === drawers
          ? drawerFronts
