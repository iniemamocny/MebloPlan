...
export function validateParts(board: Board, parts: Part[]): SheetPlan {
  ...
  for (const p0 of parts) {
    ...
    if (!board.hasGrain) {
      ...
      if (!fits)
        return {
          ok: false,
          reason: `${p.name}: ${p.w}×${p.h} > płyta ${L}×${W} (brak usłojenia, rotacja dozwolona)`,
          sheets: 0,
        };
    } else if (p.requireGrain) {
      if (p.h > L || p.w > W) {
        return {
          ok: false,
          reason: `${p.name}: ${p.w}×${p.h} przekracza ${W}×${L} (z usłojeniem: h≤${L}, w≤${W})`,
          sheets: 0,
        };
      }
    } else {
      ...
      if (!fits)
        return {
          ok: false,
          reason: `${p.name}: ${p.w}×${p.h} > płyta ${L}×${W} (rotacja dozwolona dla tej formatki)`,
          sheets: 0,
        };
    }
  }
  ...
  const area = parts.reduce(
    (s, p) => s + Math.max(1, p.qty ?? 1) * p.w * p.h,
    0,
  );
...
export function packIntoSheets(board: Board, parts: Part[]): Sheet[] {
  ...
  const prepared: PreparedPart[] = units
    .map((u) => {
...
function splitFreeRect(
  ...
  const right: FreeRect = {
    x: x + w + kerf,
    y,
    w: Math.max(0, fr.x + fr.w - (x + w + kerf)),
    h,
  };
  const bottom: FreeRect = {
    ...
    y: y + h + kerf,
    w: fr.w,
    h: Math.max(0, fr.y + fr.h - (y + h + kerf)),
  };
  const out: FreeRect[] = [];
