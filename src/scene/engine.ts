...
export function setupThree(container:HTMLElement){
  ...
  const dir = new THREE.DirectionalLight(0xffffff,0.7); dir.position.set(6,8,4); scene.add(dir)
  scene.add(new THREE.GridHelper(20,40,0xdddddd,0xcccccc))
  const floor = new THREE.Mesh(new THREE.PlaneGeometry(40,40), new THREE.MeshStandardMaterial({ color:0xeeeeee, side:THREE.DoubleSide })); floor.rotation.x=-Math.PI/2; scene.add(floor)
